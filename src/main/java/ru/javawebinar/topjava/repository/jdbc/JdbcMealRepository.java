...
public class JdbcMealRepository implements MealRepository {
    ...
    public Meal save(Meal meal, int userId) {
        ...
        if (meal.isNew()) {
            ...
            meal.setId(newId.intValue());
                ...
                "description=:description, calories=:calories WHERE id=:id", parameters) == 0) {
            return null;
        }
        return meal;
    }
    ...
    public List<Meal> getBetweenHalfOpen(LocalDateTime startDateTime, LocalDateTime endDateTime, int userId) {
        return jdbcTemplate.query("SELECT * FROM meals WHERE user_id=? AND" +
                " datetime >= ? AND datetime < ? ORDER BY datetime DESC", ROW_MAPPER, userId, startDateTime, endDateTime);
    }
}
